 ...
 * @author ADMIN
 */
public class CategoryImpl extends dao.DAO{
    public CategoryImpl() {
        super();
    }
    ...
    public Category getCategoryByProduct(int key) {
    	Category category = new Category();
        String sql = "SELECT * FROM category c,product p where c.id=p.categoryId and p.id=?"; 
        try {
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setInt(1, key);
            ResultSet rs = ps.executeQuery();
